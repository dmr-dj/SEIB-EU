...
!lai_max
   ...
   do p = 1, PFT_no
   do doy = 1, Day_in_Year
      lai_maxPFT(p) = max(lai_maxPFT(p), lai_RunningRecord(doy,p))      
   enddo
   enddo

   do p = 1, PFT_no
      npp_sumPFT(p) =  sum(npp_RunningRecord (:,p))
   enddo
!First dominant PFT
   dominant1 = 1   !initialize
   x         = 0.0 !initialize
   do p = 1, PFT_no
      if ( npp_sumPFT(p) > x ) then
         x         = npp_sumPFT(p)
         dominant1 = p
      endif
   enddo

!dmr&iab :: correct code but unused at present 2023-07-27
   
!~ ! definir variable dominancePFT(PFT_no) en entier
!~ ! definir variable logical isused = .false.

!~ dominancePFT(1) = dominant1

!~ do i =2, PFT_no
!~   xi = -999999999. !initialize

!~   do p = 1, PFT_no
!~     is_used = .false.
    
!~     do k=1,i-1
!~       if ( p .eq. dominancePFT(k)) then
!~          is_used = .true.
!~       endif
!~     enddo

!~     if ( .not. is_used ) then
!~       if ( npp_sumPFT(p) > xi ) then
!~          xi        = npp_sumPFT(p)
!~          dominancePFT(i) = p
!~       endif
!~     endif
...
!open(newunit=newunitfile,file="PFT_classement.txt", form='formatted')
!write(newunitfile,*) dominancePFT(:)
!close(newunitfile)
