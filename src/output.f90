...
!print*,"temperaure de l'air = " , tmp_air
...
!*** 4th line (Physical status of radiation)
   ...
   par_direct,',',par_diffuse,',',par*sum(par_grass_rel(:,:))/DivedG/DivedG,',',radnet_soil,',',radnet_veg, ',', radlong_up
...
!*** 2nd line (Carbon strage & flux)
   ...
   x  = C_in_drymass/Max_loc/Max_loc/100.0 !(gDM/Stand) -> (MgC/ha)
...
!_____________ Prepare output variables
   ...
   ! dmr&iab ---
   npp_PFTs(:) = sum(npp_RunningRecord(1:i, :),dim=1) * C_in_drymass / Max_loc / Max_loc / 1000.0
