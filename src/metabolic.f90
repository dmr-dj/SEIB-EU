...
!Apparent quantum yield (electron/PAR-quanta)
   ...
   (/0.1196, 0.1196, 0.1196, 0.1196, 0.1196, 0.1196, &
     ...
     0.1196, 0.0344, 0.1196, 0.1196, 0.1196,         &
     0.1196, 0.1196                                /) 
   
!Deactivation efficiencies (J mol-1)
   real,parameter,dimension(PFT_no)::&
   Deactivation_Vcmax = &
   (/193200., 193200., 193200., 193200., 193200., 193200., &
     193200., 193200., 193200.,                            &
     193200., 190000., 193200., 193200., 193200.,          &
     193200., 193200.                                     /)
...
!_____________ Preparation
  ...
  !ce_water: effect of stomata openness due to soil water shortage
   Do p=1, PFT_no
      ...
      !SEIB V2.62-V3.03
      !ce_water(p) = 2.0 * x - x ** 2.0 

!     !SEIB Ver.Farquhar
!     a           = (x-Stat_water_min(p)) /
!     abs(Stat_water_opt(p)-Stat_water_min(p))
!     b           = (Stat_water_max(p)-x) /
!     abs(Stat_water_opt(p)-Stat_water_max(p))
!     ce_water(p) = sqrt(min( 1.0, max(0.0,a), max(0.0,b) ))

      !SEIB Ver.Farquhar (TMP)
      a = stat_water(p) - Stat_water_min(p)
      b = stat_water(p) - Stat_water_max(p)
      c = stat_water(p) - Stat_water_opt(p)
      if ( a>0.0 .and. b<0.0 ) then
         !ce_water(p) = (a*b)/(a*b-c*c)
         ce_water(p) = sqrt((a*b)/(a*b-c*c))
      else
         ce_water(p) = 0.0
      endif
...
DO j=1, DivedG
   ...
   gpp_30min    = 0.0
   la_mentioned = (lai_grass(i,j) / real(DivedGrassLayer) ) * ((Max_loc/DivedG)**2) 
...
!Sumup daily 30min canopy conductance to daily canopy conductance
   ...
   canopy_cond = canopy_cond + &
                 canopy_cond_30min / Max_loc / Max_loc / TimeStepMax
...
!_____________ Preparation of PFT specific varables
   Do p=1, PFT_no
     ...
     !SEIB V3.10-
      a1          = (x-Stat_water_min(p)) / abs(Stat_water_opt(p)-Stat_water_min(p))
      a2          = (Stat_water_max(p)-x) / abs(Stat_water_opt(p)-Stat_water_max(p))
      ce_water(p) = sqrt(min( 1.0, max(0.0,a1), max(0.0,a2) ))

   End do
!____________ Compute photosynthesis hparameters for each PFT and each relative light intensity
