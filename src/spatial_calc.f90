...
!Local variables
	real,dimension(PFT_no,Max_hgt)::lai  !LAI for each PFT for each forest layer (m2/m2)
...
!_____________ Leaf Area Index for each layer for each PFT (lai)
	lai(:,:) = 0.0
	lai_cum(:,:) = 0.0
	DO no = 1, Max_no
		...
		do i = bole(no)+1, height(no)
			lai(pft(no), i) = lai(pft(no), i) + x
		end do
		do i = height(no) - 1, bole(no) + 1, -1	! lai_cum(pft(no), height(no)) = 0.0
			lai_cum(pft(no), i) = lai_cum(pft(no), i + 1) + lai(pft(no), i)
		end do
	END DO
	lai(:,:) = lai(:,:) / real(Max_loc) / real(Max_loc)
	lai_cum(:,:) = lai_cum(:,:) / real(Max_loc) / real(Max_loc)
	
!_____________ Light attenuation coefficient for each forest layer
	DO i = 1, Max_hgt
		x = 0.0
		do p = 1, PFT_no
			x = x + lai(p, i) * EK0(p)
...
!Local variables
	...
	real  frac_absorb, factor_adjustment

	real par_UnderVegetation_rel !Fraction of PAR remaining under Vegetation
	
	integer,dimension(Max_no):: tree_no
	logical,dimension(Dived,Dived):: flag
	integer count, loop_no, loop_no_max
	integer no, p, i, j
	real    dist_x, dist_y, d1, d2, d3, proxy
	real    x, y, a1, a2, a3, a4
	
ir_tree = 0.0
ir_grass = 0.0
if (par == 0.00) return

!●_____________ ir_tree: Radiation-interception-coefficient by crown layer, lower=more absorb.
	a3 = 0.0
	DO no=1, Max_no
		p = pft(no)
		if ( .not. tree_exist(no)      ) cycle
		...
		!a1: Leaf Area Density for each tree (m2/m2/STEP)
		a1 = la(no) / max(0.001,crown_area(no)) / real( max(1,height(no)-bole(no)) )
		!Fraction of absorbance of the available PAR on this layer (EK0 should be used, not eK)
		frac_absorb = 1.0 - exp(-1.000 * EK0(p) * a1)
		frac_absorb = min(1.0, max(0.0, frac_absorb ) )
		
		do i = height(no), bole(no)+1, -1
			!a2: PAR density @ this layer (micro mol photon m-2 s-1)
			a2 = par_diffuse*par_diffuse_rel(i) + par_direct*par_direct_rel(no,i) 
			
			!a3: Sumup of PAR absorbed @ this layer (micro mol photon s-1)
			a3 = a3 + (a2 * frac_absorb) * crown_area(no)
		end do
		
	END DO
	!a4: Fraction of PAR intercepted by Crown
	a4 = (a3 / real(Max_loc) / real(Max_loc) ) / par
	a4 = min( max(a4, 0.0), 1.0 )
	
	ir_tree = min( max(1.0-a4, 0.0), 1.0 )
...
!_____________ par_grass_rel !PAR on each grass cell of forest floor
...
!_____________ par_floor_rel !PAR on each establishment cell of forest floor
IF (Dived==DivedG) then
	!When grid layouts of grass cell and establishment patch cell are identical,
	!par_floor_rel(i,j) and par_grass_rel(i,j) are identical
	do i=1, Dived
	do j=i, Dived
...
!●_____________ Adjust par_grass_rel and par_floor_rel
	!a1: Average of par_grass_rel
	a1 = sum(par_grass_rel(:,:)) / real(DivedG) / real(DivedG)
	a1 = min( max(a1, 0.0), 1.0 )
	
	if (a1 > 0.001) then
		factor_adjustment = ir_tree / a1 
	else
	...
	do i=1, DivedG
	do j=1, DivedG
		par_grass_rel(i,j) = min(1.0, par_grass_rel(i,j) * factor_adjustment)
	enddo
	enddo
	
	do i=1, Dived
	do j=1, Dived
		par_floor_rel(i,j) = min(1.0, par_floor_rel(i,j) * factor_adjustment)
	enddo
	enddo
	
!●_____________ ir_grass: Radiation-interception-coefficient by grass layer, lower=more absorb.
	!p: PFT ID number
	...
	!a1: Area of a grass cell (m2)
	a1 = (real(Max_loc)/real(DivedG)) ** 2.0
	
	a3 = 0.0
	do i=1, DivedG
	do j=1, DivedG
		!Fraction of absorbance of the available PAR on this layer (EK0 should be used, not eK)
		frac_absorb =  (par_direct  / par) * exp(-1.0 * lai_grass(i,j) *  eK(p)) + &
		               (par_diffuse / par) * exp(-1.0 * lai_grass(i,j) * EK0(p))    
		frac_absorb = 1.0 - frac_absorb
		frac_absorb = min(1.0, max(0.0, frac_absorb ) )
		
		!a2: PAR density @ this grass cell
		a2 = par_grass_rel(i,j) * par
		
		!a3: Sumup of PAR intercepted (micro mol photon s-1)
		a3 = a3 + a1 * a2 * frac_absorb
	enddo
	enddo
	
	!a4: Fraction of PAR remaining under Vegetation
	a4 = ( a3 / real(Max_loc*Max_loc) ) / (ir_tree*par)
	a4 = min( max(1.0-a4, 0.0), 1.0 )
	a4 = a4 * ir_tree
	
	ir_grass = 1.0 - (ir_tree-a4) / ir_tree
...
!**************************************************************************************************
SUBROUTINE direct_radiation (LAT)
...
!_____________ Define local variables
	...
	integer loop_no1 !Required loop number of computation: for self-shading
	integer loop_no3 !Required loop number of computation: for neighbor-shading
	integer loop_no2 !Required loop number of computation: for simple computation
	
	!for geometric calculation
	real    dist, dist_x, dist_y           !distance (m)
	real    r, r1, r2                      !radius (m)
	real    y1, y2                         !
	real    x1_me,  x2_me,  y1_me,  y2_me  !coordination (m)
	real    x1_you, x2_you, y1_you, y2_you !coordination (m)
	real    y_shift, xband_me, xband_you   !coordination (m)
...
!_____________ Following procedures required only if sunlight exist
	...
	!Shade length for single height step (unit=m/STEP)
...
!_____________ Select trees that require computations
	DO no = 1, Max_no
		...
		!Trees that require intensive computation
		...
		!Trees that require simple computation
...
!_____________ Simple computation for light attenuation
	If (loop_no2 >= 1) then
...
!_____________ Intensive computation of self-shading
	...
	DO loop = 1, loop_no1             !For each tree
		...
		Do i = 1, height(me)-max(bole(me),Hgt_thres)-1 !subtraction of 1 is due to top layer
...
!_____________ List up tree-pairs that interfere
	...
	Do j = 1, loop_no1 !tree 'you'
		...
		!Cancel no-interference trees 1
		if ( bole(me)+1 >= height(you) ) cycle
		if ( i == j )                    cycle
		
		!Cancel no-interference trees 2
		...
		!Cancel no-interference trees 3
...
!		if (LAT + sl_dec(doy) > 0) then !正しい計算はこちらだが、何故かセグメント違反になる
...
!		if (LAT + sl_dec(doy) > 0) then !正しい計算はこちらだが、何故かセグメント違反になる
	...
	!reflect shading to par_direct_rel for each crown disk
	do i = max(Hgt_thres+1, bole(me)+1), min(height(me), height(you)-1)
		!i: height of current mentioned crown, Absolute value
		l1 = max(1,   bole(you) + 1 - i) !lower, Relative value
		l2 = max(1, height(you)     - i) !upper, Relative value
...
Do no=1, Max_no
	...
	!examine for proximate area for each grid of establishment coordinate
	do i = 1, Dived
	do j = 1, Dived
		
		!location of this tree under normal coordinate
...
!**************************************************************************************************
! Calculate spacial limitation on growth (for woody PFTs, Monthly computation)
!**************************************************************************************************
...
!Local parameters
	...
	real   ,parameter::Crown_plasticity  = 0.75 !maximum fraction of crown movement (fraction)
...
!Local variable
	!maximum length of crown movement for each direction (m)
...
!   output --> proxy_crown (Max_proxy, 3), count1
	...
	DO you = 1, Max_no
		...
		DO i=1, 9
			...
			!in this case, this tree can interfere, and thus its information is saved
			count1 = count1 + 1
...
!   output --> lenght_max2 (Direct_resolusion)
	...
	DO count2=1, count1
		...
		DO i=1, Direct_resolusion
			...
			!covert target crown location on the new coordinate system
